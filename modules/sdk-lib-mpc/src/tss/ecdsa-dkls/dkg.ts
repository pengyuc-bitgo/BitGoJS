...
export class Dkg {
  ...
  constructor(n: number, t: number, partyIdx: number, retrofitData?: RetrofitData) {
    ...
    if (retrofitData) {
      ...
      const big_s_list: Array<Array<number>> = [];
      let j = 0;
      for (let i = 0; i < n; i++) {
        if (i === partyIdx) {
          const secp256k1 = new Secp256k1Curve();
          big_s_list.push(Array.from(bigIntToBufferBE(secp256k1.basePointMult(BigInt('0x' + retrofitData.xShare.x)))));
        } else {
          big_s_list.push(Array.from(Buffer.from(retrofitData.bigSiList[j], 'hex')));
          j++;
        }
      }
